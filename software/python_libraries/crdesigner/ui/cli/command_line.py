...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('-p1', '--points_file_one', type=str, help='Path to the first file containing points', default=None)
    parser.add_argument('-p2', '--points_file_two', type=str, help='Path to the second file containing coordinates', default=None)
...
def main():
    ...
    first_points_list = retrieve_points(args.points_file_one)
    second_points_list = retrieve_points(args.points_file_two)
    if args.mode == "gui":
        start_gui(input_file, first_points_list, second_points_list)
    else:
...
# Reads points from an .xml file stored in the path 
def retrieve_points(path):
    if not path:
        return None
    try:
        print(path)
        ...
        print("Couldn't read the {} file properly:".format(path), e)
